...
pub async fn run_stdio_server(args: AdmiralMcpServerArgs) -> Result<(), String> {
    let (caller, _root_guard, _driver_task) =
        ship_mcp::connect_to_ship(&args.server_ws_url, "admiral-mcp", &args.session_id.0)
            .await
            .map_err(|e| e.to_string())?;

    let client = AdmiralMcpClient::from(caller);
    ...
    ship_mcp::admiral_server(ctx)
        .run()
        .await
        .map_err(|e| format!("admiral MCP server failed: {e}"))
}
