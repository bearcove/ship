...
export type SessionEvent =
  ...
  | { tag: "TaskStarted"; task_id: TaskId; title: string; description: string }
...
export class ShipDispatcher implements ChannelingDispatcher {
  ...
  async dispatch(method: MethodDescriptor, args: unknown[], call: RoamCall): Promise<void> {
    ...
    } else if (method.id === 0x19b6f89c7698a334n) {
      try {
    ...
    } else if (method.id === 0x95310d9af11d8d0fn) {
      try {
    ...
    } else if (method.id === 0x0ad58ffc1cc60629n) {
      try {
...
const ship_schema_registry: SchemaRegistry = new Map<string, Schema>([
  ...
  [
    "AgentDiscovery",
    { kind: "struct", fields: { claude: { kind: "bool" }, codex: { kind: "bool" } } },
  ],
  [
    "ServerInfo",
