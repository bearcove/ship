...
export type ListWorktreeFilesRequest = [SessionId];
...
export class ShipDispatcher implements ChannelingDispatcher {
  ...
  async dispatch(method: MethodDescriptor, args: unknown[], call: RoamCall): Promise<void> {
    ...
    } else if (method.id === 0x4cd594abe50380aan) {
      try {
    ...
    } else if (method.id === 0x9125e15a6d072025n) {
      try {
        const result = await this.handler.listWorktreeFiles(args[0] as SessionId);
        call.reply(result);
      } catch {
        call.replyInternalError();
      }
    ...
    } else if (method.id === 0x525fdefefed33dbdn) {
      try {
