...
export class ShipDispatcher implements ChannelingDispatcher {
  ...
  async dispatch(method: MethodDescriptor, args: unknown[], call: RoamCall): Promise<void> {
    ...
    } else if (method.id === 0x76a41698f5b3e807n) {
      try {
    ...
    } else if (method.id === 0xd1356811bcd7ace5n) {
      try {
    ...
    } else if (method.id === 0xc01d52323dacd757n) {
      try {
...
const ship_schema_registry: SchemaRegistry = new Map<string, Schema>([
  ...
  ["AgentSnapshot", { kind: 'struct', fields: { 'role': { kind: 'ref', name: 'Role' }, 'kind': { kind: 'ref', name: 'AgentKind' }, 'state': { kind: 'ref', name: 'AgentState' }, 'context_remaining_percent': { kind: 'option', inner: { kind: 'u8' } }, 'model_id': { kind: 'option', inner: { kind: 'string' } }, 'available_models': { kind: 'vec', element: { kind: 'string' } } } }],
